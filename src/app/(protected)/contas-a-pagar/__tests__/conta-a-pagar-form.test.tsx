...
jest.mock("@/services/contas-a-pagar-service", () => ({
  contasAPagarService: { cadastrar: jest.fn(), alterar: jest.fn() },
}));
...
describe("ContaAPagarForm - valor da parcela", () => {
  it("não deve exibir label de valor da parcela quando parcelado está desativado", async () => {
    ...
    await user.type(screen.getByRole("spinbutton", { name: "Valor" }), "500");
  ...
  it("deve exibir o valor correto da parcela quando valor e quantidade são preenchidos", async () => {
    ...
    await user.type(parcelasInput, "4");

    expect(
      screen.getByText("Valor de cada parcela: R$ 125,00"),
    ).toBeVisible();
  });
});
