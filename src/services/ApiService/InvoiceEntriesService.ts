...
export const invoiceEntriesService = {
  ...
  create: (data: IInvoiceEntryCreateData): Promise<AxiosResponse<IInvoiceEntry>> => apiClient.post(`/cards/${data.card_id}/entries`, data),
  update: (values: IInvoiceEntryUpdateData): Promise<AxiosResponse<IInvoiceEntry>> => apiClient.put(`/invoice-entries/${values.id}`, values.data),
