    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger>
        <Button variant="icon" aria-label="Notificações">
          <Icon icon={Bell} loading={isFetching} />
      ...
      <DropdownMenu.Content align={align} className="w-72 md:w-80">
        ...
        <div className="max-h-76 overflow-y-auto overflow-x-hidden divide-y divide-gray-700/40">
          {notificacoes.length === 0 ? (
            ...
            notificacoes.map((notificacao) => (
              <DropdownMenu.Item key={notificacao.id}>
                <Link
                  href={`#`}
                  className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-primary/8 rounded-lg transition-colors duration-150"
                >
        ...
        {notificacoes.length > 0 && (
          <div className="px-2 pt-2 border-t border-gray-700/50 mt-1">
